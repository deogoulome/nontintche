...
export const initierPaiement = async (req, res) => {
  ...
  try {
    ...
    const utilisateur = await prisma.utilisateur.findUnique({ where: { id: id_utilisateur } })
    ...
    const transaction = await Transaction.create({
      ...
      customer: {
        ...
        phone_number: {
          number: '64000001', // TODO: remplacer par utilisateur.telephone en production
          country: 'BJ'
        }
      }
    })
    ...
    console.error('Erreur FedaPay initierPaiement:', err)
    res.status(500).json({ message: 'Erreur lors de l\'initiation du paiement' })
  }
}

// Vérifier le statut d'une transaction après retour utilisateur
export const verifierPaiement = async (req, res) => {
  ...
  try {
    ...
    console.error('Erreur FedaPay verifierPaiement:', err)
